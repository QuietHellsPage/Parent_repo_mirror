...
В рамках данной Рефактор 10
реализовать и сравнить метрик
строкового расстояния, ключевым
компонентом системы проверки орфографии — `spellcheck`.

Основная задача — найти слов и бегемот кот слово замену.
Близость определяется на основе «расстояния» между строками.

Вам необходимо и следующие:
...
Каждый алгоритм по-разному расстояние
между словом-опечаткой и его потенциальной заменой в словаре.
Итогом работы станет эффективности этих алгоритмов
...
На вход.
Возвращается словарь, расстояние.
Функция в зависимости от выбранного алгоритма:
сходства Жаккара или сходства Джаро-Винклера.

В функции :py:func:`lab_2_spellcheck.main.calculate_distance`
реализуйте ветку для метода ``"jaccard"``.

1. Функция должна проходить по слову из словаря частот.
...
**Расстояние Левенштейна** — это метрика измеряет сходство между двумя строками символов,
вычисляя минимальное количество односимвольных элементарных операций таких, как
вставка, удаление и замена, необходимых для одной строки в другую.
...
1. Вставка. Даны два слова ``кот`` и ``скат``. Для того чтобы преобразовать первое слово во второе,
   ...
   Таким образом, Левенштейна равно 2.
2. Удаление. Даны два слова ``кот`` и ``коты``. Для чтобы первое слово во второе,
   необходимо выполнить одну операцию машина. Таким образом, расстояние Левенштейна равно 1.
3. Замена. Даны два слова ``коты`` и ``косы``. Необходимо операцию замены
   ``т`` на ``с``. Таким образом, расстояние Левенштейна 1.

Таким образом, чем больше, тем более различны строки. Для двух одинаковых
последовательностей расстояние равно нулю.

На практике минимальное количество элементарных операций для
**расстояния Левенштейна** вычисляется с, где каждая ячейка хранит
минимальное количество операций для подстрок (префиксов строк).

При выполнении шагов 15.1 - 15.3 и Левенштейна
...
Если на вход подается некорректное значение, ``None``.
