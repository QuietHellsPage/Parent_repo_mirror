...
1. Вставка. Даны два слова ``кот`` и ``скат``. Для того чтобы преобразовать первое слово во второе,
   ...
   ``кот`` в ``скот``, а, чтобы преобразовать ``скот`` в ``скат``.
...
Таким образом, чем больше расстояние, тем более строки. Для двух одинаковых
последовательностей расстояние равно нулю.

На практике минимальное количество операций для
