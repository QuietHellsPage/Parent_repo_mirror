...
def get_json_from_source(source_ref: str, target_repo: str) -> tuple[Optional[dict], bool]:
    ...
    stdout, _, rc = run_git(["rev-parse", f"{source_ref}:{TRACKED_JSON_PATH}"], cwd=target_repo, check=False)
    if rc == 0:
        source_sha = stdout.strip()
    else:
        logger.info("JSON file not found in source ref %s", source_ref)

    stdout, _, rc = run_git(["rev-parse", f"origin/main:{TRACKED_JSON_PATH}"], cwd=target_repo, check=False)
    if rc == 0:
    ...
    if source_sha is not None:
        stdout, _, rc = run_git(["show", f"{source_ref}:{TRACKED_JSON_PATH}"], cwd=target_repo, check=False)
        if rc == 0 and stdout:
...
def sync_files_from_source(
    ...
    for source_path, target_path in sync_list:
        stdout, _, rc = run_git(["show", f"{source_ref}:{source_path}"], cwd=repo_path, check=False)
    ...
    if json_content:
        ...
        for source_path, target_path in sync_mapping:
            source_sha, _, rc_src = run_git(
                ["rev-parse", f"{source_ref}:{source_path}"], cwd=target_repo, check=False
            )
            if rc_src != 0:
                source_sha = None

            target_sha, _, rc_tgt = run_git(
                ["rev-parse", f"origin/main:{target_path}"], cwd=target_repo, check=False
            )
            if rc_tgt != 0:
                target_sha = None
