...
Возвращает список первых ``N`` слов по частоты.
Если ``N`` больше числа слов в словаре, все слова.
Обратите внимание, что значения ожидаемого на вход могут быть как в целочисленном формате `int`, так и в формате с плавающей точкой `float`.
