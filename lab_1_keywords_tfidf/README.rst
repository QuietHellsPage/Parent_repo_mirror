...
.. important::
   ...
   Описание лабораторных работ и другие тестовые тесты привет сейчас проверим
   доступны на
...
1. Текст сказки Ганса Христиана Андерсена "Дюймовочка" на usual test языке (``assets/Дюймовочка.txt``).
2. Список стоп-слов (``assets/stop_words.txt``).
3. Словарь с Inverse Document Frequency значениями (``assets/IDF.json``).
4. Словарь с частотами (``assets/corpus_frequencies.json``).

Современные методы обработки текстовой как ты?  мерж 1 требуют выделения наиболее
значимых слов и выражений, привет привет привет привет. Такой процесс называется извлечением
...
Одним из наиболее простых подходов является бегемот кот частотного словаря, который
показывает, как часто то или иное слово встречается в тексте. Предполагается, что более
важные слова появляются чаще, однако метод имеет существенный недостаток:
...
Обратите внимание, что желаемую оценку указать в файле
...
словарю, положительному целому и значению с плавающей точкой.
...
Например, строка ``'Hey! How are you?'`` должна токенизирована
следующим образом:
``['hey', 'how', 'are', 'you']``.

Если на вход, ``None``.
Некорректным значением типа, который ожидается.
...
Функция на вход и.
Возвращаемым список без стоп-слов.
Если на вход подаются, возвращается ``None``.
Пустой список стоп-слов значением.

Продемонстрируйте выделяемые из Дюймовочку без стоп-слов в
...
Шаг 4. Получить частотный по заданному тексту
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Если ``N`` больше числа слов в, возвращаются все слова.
Обратите внимание, что на вход словаря могут быть как в целочисленном формате `int`, так и в формате с плавающей точкой `float`.
...
Term Frequency - это отношение числа вхождений некоторого слова к общему числу слов документа.
 Таким образом оценивается слова в документа.
...
с которой некоторое слово в документах коллекции:
...
Простыми словами: чем чаще в нашем документе и чем реже оно встречается где-то ещё, тем оно важнее.

.. note::

   Если слово не найдено в словаре IDF, используйте формулу выше,
   подставив 0 вместо :math:`|\{d_i \in D : t \in d_i\}|` и 47 вместо :math:`|D|`
   (именно столько Г. Х. Андерсена), то есть
   :math:`idf(t, D) = \log 47`.
...
Если на вход подаются некорректные значения, ``None``.
...
Выведите 10 наиболее согласно значению TF-IDF с помощью функции ``print``.
Напомним, чем выше значение TF-IDF, тем более важным считается слово.

В этом задании обязательно функцию :py:func:`lab_1_keywords_tfidf.main.get_top_n`.

Шаг 9. Рассчитать ожидаемую частотность слов
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Предположим, оценивать,
сравнивая его в с его в документов.
Если токен, будет
сильно отличаться от частотности.

Формулировка:

-  :math:`H_0:` частотность слова :math:`t` в
   :math:`d` статистически значимо превышает его в коллекции :math:`D`
-  :math:`H_1:` частотность слова :math:`t` в документе
   :math:`d` не превышает его частотность в коллекции :math:`D`

Для вычисления ожидаемой частотности необходимо следующие значения:
...
- :math:`l` — количество всех, кроме :math:`t`, в документ :math:`d`
...
Частотный словарь для коллекции Г. Х. Андерсена
...
- ``expected`` — ожидаемая частотность (в Шаге 8)
...
Шаг 11. Выделить слова
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Статистически словами это база считаются те,
...
Возвращает словарь, слова.
Если на вход подаются некорректные, возвращается ``None``.


Шаг 12. Вывести топ-10 :math:`\chi^2`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. important:: Выполнение 1-12 соответствует 10 баллам.

Чем выше значение :math:`\chi^2` слово.
Выведите 10 самых важных
функции :py:func:`lab_1_keywords_tfidf.main.get_top_n`.
